...
class ExitController {
	async index(req, res) {
		...
		if (startDate || finalDate) {
			...
			const startDate = moment(req.body.startDate).format("YYYY-MM-DDT00:mm:ss.SSSZ");
			const finalDate = moment(req.body.finalDate).format("YYYY-MM-DDT23:59:ss.SSSZ");

			filters.date.$gte = startDate;
			filters.date.$lte = finalDate;
		} else {
			// Consulta todas as saídas se as datas não forem fornecidas
			allExits = await Exit.find({}).sort("-date").exec();
		}
	...
	async createExit(req, res) {
		...
		const exit = new Exit({
			descriptionExit: descriptionExit,
